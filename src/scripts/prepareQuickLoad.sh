...
#   name=${bedfile%.bed.gz}
#   echo $name
#   zcat $bedfile |grep -v 'track name' | sort -k1,1 -k2,2n | perl -p -e 's/^([0-9XYN]+[\t\ ])/chr$1/g' > sorted.bed
#   outname=`echo $name`.bb   
#   ./bedToBigBed  -extraIndex=name -type=bed9 sorted.bed hg19.chrom.sizes $outname
#done
...
cd ..
#wget --no-directories   -r --no-parent -A '*dense.bed.gz'  http://egg2.wustl.edu/roadmap/data/byFileType/chromhmmSegmentations/ChmmModels/coreMarks/jointModel/final/
