...
public class AnnotationsFromTrack {
	...
	public Collection<String> getAnnotationsByNumberOfOverlap() {
		...
		for (Integer count: counts) {
			for (String annotation: getAnnotations()) {
				if (annotationsMappedCount.get(annotation).equals(count)  && false == sortedAnnotations.contains(annotation)) {
					sortedAnnotations.add(annotation);
				}
			}
		}
	...
	public void process(String trackId, Collection<TopkResult> results) {
												...
												while (nextResult || nextAnnotation) {
													...
													if (nextAnnotation) {
														...
														if ( null != ((SymWithProps) seqSym).getProperty("name") ) {
															...
															for (Object propertyValue : ((SymWithProps) seqSym).getProperties().values()) {
		...
		logger
				.info("Num annotations: " + this.getAnnotations().size() + " / " + annotationsCount.keySet().size());
	...
	private void increase(HashMap<String, Integer> annotationsCount, String annotation) {

		if (annotationsCount.containsKey(annotation)) {
			annotationsCount.put(annotation, annotationsCount.get(annotation) + 1);
		} else {
			annotationsCount.put(annotation, 1);
		}
	}
